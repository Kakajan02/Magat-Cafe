        ...
        <div class="row g-4">
            ...
            <div class="col-12 flex flex-row justify-between">
                ...
                <div class="col-4">
                    <div class="sp-plan-opt">
                      <div class="custom-control custom-switch">
                        <input onclick="ingradient()" type="checkbox" class="custom-control-input @error('has_parent') is-invalid @enderror" name="has_parent"
                               id="has-parent" {{ old('has_parent') ? 'checked' : '' }}>
                        <label class="custom-control-label text-soft" for="has-parent">@lang('main.add') @lang('main.ingredient')</label>
                        @error ('has_parent')
                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                      </div>
            ...
            <div id="ingradient" style="position: relative;" class="col-md-12 ingradientActive card card-bordered mb-2 mt-4">
                <div class="relative card-inner col-md-12">
                    <h5 class="float-title">@lang('main.ingredient')</h5> 
                    <div style="display:flex;flex-flow:row;justify-content:space-between; border-bottom:2px solid #ddd; padding-bottom:5px;margin-bottom:10px;align-items:center;" class="col-12">
                        <div>
                            <p style="color:#8bc34a;font-size:18px;user-select: select none;">@lang('main.add_ingredient')</p>
                        </div>
                        <div onclick="addInput(langs)" class="addField add" >
                            <div>&plus;</div>
                        </div>
                    </div>
                    {{-- inp-group clasy jsde element doretmek un ulanylyar start --}}
                    <div class="inp-group">   

                    </div>
                    {{-- end asaky komentaryadaky konteynerler doreyar --}}


                    {{-- <div class="flex flex-row justify-start mt-2">
                        @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            <div style="width: auto; margin:0 4px;">
                                <div class="form-group">
                                    <label for="ingradient-{{ $localeCode }}"
                                        class="form-label">{{ $properties['native'] }}</label>
                                    <div class="form-control-wrap">
                                        <input type="text" id="ingradient-{{ $localeCode }}" name="ingradient[{{ $localeCode }}]"
                                            value="{{ old('ingradient.' . $localeCode) }}"
